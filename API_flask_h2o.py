...
col_dict = {'RevolvingUtilizationOfUnsecuredLines': 'real',
            'age': 'int',
...
class LoanArrearsForecast(Resource):
    ...
    def get(self):
        ...
        # получаем входные аргументы с помощью обекта парсера
        args = parser.parse_args()
        # извлекаем значения соответствующих переменных
        RevolvingUtilizationOfUnsecuredLines = float(args['RevolvingUtilizationOfUnsecuredLines'])
        age = int(args['age'])
        NumberOfTime30_59DaysPastDueNotWorse = int(args['NumberOfTime30_59DaysPastDueNotWorse'])
        DebtRatio = float(args['DebtRatio'])
        MonthlyIncome = float(args['MonthlyIncome'])
        NumberOfOpenCreditLinesAndLoans = int(args['NumberOfOpenCreditLinesAndLoans'])
        NumberOfTimes90DaysLate = int(args['NumberOfTimes90DaysLate'])
        NumberRealEstateLoansOrLines = int(args['NumberRealEstateLoansOrLines'])
        NumberOfTime60_89DaysPastDueNotWorse = int(args['NumberOfTime60_89DaysPastDueNotWorse'])
        ...
        # преобразуем данные в датафрейм h2o, используя словарь c описанием типов столбцов
        input_h2o_df = h2o.H2OFrame(input_df, column_types=col_dict)

        # подаем данные на вход нашей модели, получаем прогноз и преобразуем его в датафрейм pandas
        predictions = uploaded_model.predict(input_h2o_df).as_data_frame()
...
if __name__ == '__main__':
    ...
    # (при окончательном запуске в прод-среде режим отладки нужно отключить)
